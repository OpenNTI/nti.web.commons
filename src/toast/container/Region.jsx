...
const getTransitionPrefix = location =>
	transitionMap[location]?.toLowerCase() ?? location.toLowerCase();
const getTransitionName = location =>
	`${getTransitionPrefix(location)}-transition`;
...
export default function TileContainerRegion({ location, toasts }) {
	...
	return (
		<TransitionGroup
			component="ul"
			className={cx(styles.region, styles[regionName] || regionName)}
		>
			{toasts.map(toast => (
				<CSSTransition
					key={toast.id}
