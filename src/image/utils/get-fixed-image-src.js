...
const LetterBoxAppliers = {
	...
	src: (ctx, image) => {
		...
		const actualScale =
			imageWidth === width ? height / imageHeight : width / imageWidth;
...
function applyLetterBox(ctx, image, type) {
	...
	const applier =
		LetterBoxAppliers[type] || LetterBoxAppliers[DefaultLetterBox];
...
export default function getFixedImageSrc(image, aspectRatio, letterbox) {
	...
	if (fitsAspectRatio(image, aspectRatio)) {
		return image.src;
	}

	const { naturalWidth: imageWidth, naturalHeight: imageHeight } = image;

	const { width, height } = fixDimensions(
		imageWidth,
		imageHeight,
		aspectRatio
	);
