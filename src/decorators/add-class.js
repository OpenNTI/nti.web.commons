...
class AddClassToBody extends React.Component {
	...
	render () {
		const {_component:Component, _forwardRef: ref, ...otherProps} = this.props;
...
export default function addClass (node, className) {
	if (!node) { throw new Error('addClass decorator must be provided a node to add the class to.'); }
	if (!className) { throw new Error('addClass decorator must be provided a class to add.'); }
