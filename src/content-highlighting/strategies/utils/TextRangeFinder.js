...
export function indexText (node, nodeFilterFn) {
	...
	for (;;) {
		while (iNode < nNodes) {
			...
			if (child.nodeType === 3) {
				if (nodeFilterFn && !nodeFilterFn(child)) { continue; }
...
export function mapMatchToTextRange (match, whichGroup, textIndex, doc) {
	...
	let entryRightIndex = searchForEntry(entryLeftIndex, indices.length, textEnd, indices, true);
...
const normalizeTextIndex = (node, doc, searchFor, which, textIndex) => textIndex ?? indexText(node, allButObjects);
...
export function findTextRanges (...args) {
	...
	for (;;) {
		...
		for (let whichGroup of which) {
			try {
				const range = mapMatchToTextRange(matchingText, whichGroup, textIndex, doc);
