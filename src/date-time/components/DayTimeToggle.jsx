...
const DEFAULT_TEXT = {
	'date-error-availableBeginning':
		'Begin Date cannot come after Finish Date.',
...
export default class DayTimeToggle extends React.Component {
	...
	componentDidUpdate(prevProps) {
		if (
			prevProps.availableBeginning !== this.props.availableBeginning ||
			prevProps.availableEnding !== this.props.availableEnding
		) {
	...
	onToggle = ({ target }) => {
		...
		if (
			(!this.state[TOGGLE.BEGIN] && active === TOGGLE.BEGIN) ||
			target.getAttribute('name') === active
		) {
			return;
		}
	...
	render() {
		...
		const {
			availableBeginning: begin,
			availableEnding: end,
			disableText,
		} = this.props;

		const trigger = (
			<DayTimeToggleTrigger
				availableBeginning={begin}
				availableEnding={end}
		...
		const beginClassNames = cx('part beginning', {
			active: active === TOGGLE.BEGIN,
		});
		const endClassNames = cx(
			'part ending',
			{ active: active === TOGGLE.END },
			{ disabled: !this.state[TOGGLE.BEGIN] }
		);
		const btnClassNames = cx('flyout-fullwidth-btn', {
			changed: changed,
			error,
		});
		...
		return (
			...
			>
				...
				<div className={btnClassNames} onClick={this.onSave}>
					Save
				</div>
			</Flyout.Triggered>
		);
