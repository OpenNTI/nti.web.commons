...
describe('DateTimeField', () => {
	...
	test('Base case: Nothing Passed - Value to be undefined', () => {
		testRender({ onChange: () => {} }).forEach(x => {
			expect(
				[...x.container.querySelectorAll('select')].map(_ => _.value)
	...
	test('on date change is called with clicked month', () => {
		...
		expect(cmp.onDateChange).toHaveBeenCalledWith(
			event.target.value,
			'setMonth'
		);
	});
	...
	test('clicking current date calls onchange with current date', () => {
		...
		testRender({ onChange: onChangeSpy, currentDate: true }).forEach(x => {
			const currentDate = x.container.querySelector(
				'.set-current-date a'
			);
	...
	test('Selecting a month updates every select box', () => {
		...
		testRender({ onChange: onChangeSpy }).forEach(x => {
			const monthSelect = x.container.querySelector(
	...
	test('updating when a prop value was intially passed', () => {
		...
		testRender({ onChange: onChangeSpy, value }).forEach(x => {
			const monthSelect = x.container.querySelector(
				'.month-wrapper Select'
			);
			const event = { target: { value: '7' } };
	...
	test('change the time updates the time and keeps the date', () => {
		...
		testRender({ onChange: onChangeSpy }).forEach(x => {
			const minuteInput = x.container.querySelector(
				'input[name=minutes]'
			);
	...
	test('render the correct day options', () => {
		...
		testRender({ value, onChange: () => {} }).forEach(x => {
	...
	test('render the current year and five subsequent years by default', () => {
		...
		testRender({ value: null, onChange: () => {} }).forEach(x => {
			const yearSelect = x.container.querySelector(
				'.year-wrapper Select'
			);

			// all the correct years
			Array.from({ length: numYears }).forEach((_, i) =>
				expect(
			...
			expect(
				yearSelect.querySelector(
					`option[value="${currentYear + numYears}"]`
				)
			).toBeFalsy();
		});
	});
	...
	test('render the correct years when passed startYear and numYears', () => {
		...
		testRender({
			...
			Array.from({ length: numYears }).forEach((_, i) =>
				expect(
					yearSelect.querySelector(`option[value="${startYear + i}"]`)
				).toBeTruthy()
			);

			// not earlier
			expect(
				yearSelect.querySelector(`option[value="${startYear - 1}"]`)
			).toBeFalsy();

			// not later
			expect(
				yearSelect.querySelector(
					`option[value="${startYear + numYears}"]`
				)
			).toBeFalsy();
	...
	test('show the error message passed to it', () => {
		...
		testRender({ onChange: () => {}, error: errorMessage }).forEach(x => {
	...
	test("include an option for the specified value's year when it's outside regular option bounds", () => {
		...
		testRender({ value, onChange: () => {} }).forEach(x => {
			...
			expect(
				yearSelect.querySelector('option[value="1980"]')
			).toBeTruthy();
		});
		...
		testRender({ value, onChange: () => {} }).forEach(x => {
			...
			expect(
				yearSelect.querySelector('option[value="2038"]')
			).toBeTruthy();
		});
	});
});
