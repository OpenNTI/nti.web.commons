	...
	render() {
		...
		const mainText =
			!begin && !end
				...
				: begin && end
				? `${format(begin, MONTH_NAME_DAY)} - ${format(
						end,
						MONTH_NAME_DAY
				  )}`
		...
		return (
			<div
				{...otherProps}
				className={cx('daytime-toggle-trigger', className)}
				ref={this.domNode}
			>
				...
				<div className="main">
					<div className={mainTextClassNames}>
						{mainText}
						<div className={clearClassNames} onClick={onChange}>
							Clear
						</div>
					</div>
				</div>
