...
function renderItem(item, key, remaining) {
	...
	return (
		<span className={cls} key={key} title={title}>
			{item}
		</span>
	);
}
...
export const RENDERERS = {
	...
	['{list}'](list, remaining, getString, renderOverrides, key) {
		...
		return list.reduce((acc, item, index) => {
			...
			if (index !== 0) {
				acc.push(
					<span className="separator" key={`${itemKey}-seperator`}>
						{getString('separator')}
					</span>
				);
			}
	...
	['{remaining}'](list, remaining, getString, renderOverrides, key) {
		...
		return (
			remaining && (
				<span className="remaining" key={key}>
					{getString('remaining', { count: remaining })}
				</span>
			)
		);
	},
