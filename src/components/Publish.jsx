...
const DEFAULT_TEXT = {
	lesson: {
		...
		schedule: {
			...
			selectedText:
				'Lesson contents will be visible to students on %(date)s at %(time)s.',
			label: 'Schedule',
	...
	survey: {
		...
		schedule: {
			...
			selectedText:
				'Survey contents will be visible to students on %(date)s at %(time)s.',
			label: 'Schedule',
			timePickerHeader: 'YOUR LOCAL TIME',
		},
	},
...
export default class Publish extends React.Component {
	...
	static evaluatePublishStateFor(object) {
		if (
			typeof object.isPublished !== 'function' ||
			typeof object.getPublishDate !== 'function'
		) {
			throw new TypeError(
				'Argument does not conform to expected interface!'
			);
		}
		...
		if (isPublished || hasPublishDatePassed) {
			...
			logger.warn(
				'Not expected. The node should be published if its lesson is published: %o',
				object
			);
		}
	}
	...
	static propTypes = {
		...
		localeContext: PropTypes.oneOf(['lesson', 'assignment', 'survey']),
	};
	...
	onChange = e => {
		...
		const scheduleChange =
			oldSelected === PUBLISH_STATES.SCHEDULE &&
			changed === true &&
			selected === PUBLISH_STATES.SCHEDULE;
	...
	render() {
		...
		const saveClassNames = cx('flyout-fullwidth-btn', {
		...
		return (
			...
			>
				<Radio
					name="publish-radio"
					value={PUBLISH_STATES.PUBLISH}
					label={t('publish.label')}
					checked={PUBLISH_STATES.PUBLISH === selected}
				...
				<Radio
					name="publish-radio"
					value={PUBLISH_STATES.SCHEDULE}
					label={t('schedule.label')}
					checked={PUBLISH_STATES.SCHEDULE === selected}
					onChange={this.onChange}
				>
					{dayClicked
