...
function getFileDragImage(count) {
	...
	return FileDragImageTemplate.replace(/\{count\}/g, count)
...
export default class Entity extends React.Component {
	...
	onDragStart = e => {
		...
		const dragging = selection.isSelected(item)
	...
	onDrop = e => {
		...
		if (isFile || isSelected || isBegingDragged) {
			logger.debug(
				'Ignoring Drop. isFile: %s, isSelected: %s, isBegingDragged: %s',
				isFile,
				isSelected,
				isBegingDragged
			);
	...
	onBeginRename = e => {
		...
		if (item.can('rename')) {
			...
			const rename =
				(item.isFolder ? filename : path.basename(filename, ext)) ||
				filename;
	...
	onSelect = e => {
		...
		if (!Events.isActionable(e)) {
			return;
		}

		if (!shiftKey) {
			//if shift was pressed, let this bubble up
			e.preventDefault();
			e.stopPropagation();
		}
	...
	onTrigger = e => {
		...
		if (e.metaKey || e.ctrlKey || this.state.rename) {
			return;
		}
