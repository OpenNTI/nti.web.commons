...
export default class Label extends React.Component {
	...
	render() {
		const {
			validity,
			props: { className, label, error, ...otherProps },
			interacted,
		} = this;
		...
		return (
			<label
				{...otherProps}
				className={cls}
				onInput={this.onInputOrFormValidation}
				onInvalid={this.onInputOrFormValidation}
				aria-live="polite"
			>
				{label && <Text.Base className="label">{label}</Text.Base>}
				{this.renderInput()}
				{error && (
