...
export default class NTIRGBInput extends React.Component {
	...
	onRedChange = (red, e) => {
		...
		const newValue = value ? value.rgb.setRed(red) : Color.fromRGB(red, 0, 0);
	...
	onGreenChange = (green, e) => {
		const {onChange, value} = this.props;
		const newValue = value ? value.rgb.setGreen(green) : Color.fromRGB(0, green, 0);
	...
	onBlueChange = (blue, e) => {
		const {onChange, value} = this.props;
		const newValue = value ? value.rgb.setBlue(blue) : Color.fromRGB(0, 0, blue);
	...
	renderInput (value, onChange, placeholder) {
		return (
			<NumberInput className={cx('nti-rgb-number-input')} value={value} onChange={onChange} min={0} max={255} constrain step={1} placeholder={placeholder} />
		);
	}
}