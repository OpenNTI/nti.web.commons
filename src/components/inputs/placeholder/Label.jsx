...
export default function Label ({className, variant = 'box', error, noError, children, ...remainingProps}) {
	...
	const inputListener = useCallback(e => setEmpty(!e.target.value), [setEmpty]);
	...
	return (
		<Frame className={cx(styles.container, className)} empty={empty} error={errorMessage} data-input-id={id} {...remainingProps} >
			...
			{label && (
				<Text.Base as="label" data-placeholder htmlFor={id}>{label}</Text.Base>
			)}
		</Frame>
	);
}
