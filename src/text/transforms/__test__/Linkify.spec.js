...
const Inner = React.forwardRef((props, ref) => (
	<InnerWrapper {...props} innerRef={ref} />
));
	...
	describe('statics', () => {
		describe('shouldApply', () => {
			test('true if linkify and not hasComponents', () => {
				expect(
					Linkify.shouldApply({ linkify: true, hasComponents: false })
				).toBeTruthy();
			});
			...
			test('false if linkify and hasComponents', () => {
				expect(
					Linkify.shouldApply({ linkify: true, hasComponents: true })
				).toBeFalsy();
			});
		});
	});
