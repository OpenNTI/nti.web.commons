	...
	const labels = [
		(
			<div className={cx('label')} key="errors" >
				...
				<Text.Base className={cx('errors')} getString={t} localeKey="errors" with={{count: types.errors.length}} />
			</div>
		)
	];
	...
	if (types.warnings.length > 0) {
		labels.push(
			<div className={cx('label', 'warning')} key="warnings">
				<Icons.Alert className={cx('icon')} />
				<Text.Base className={cx('warnings')} getString={t} localeKey="warnings" with={{count: types.warnings.length}} />
			</div>
		);
	}
